...
initm=10
test_size=0.2
GWL_type= "mixed" #annual, inertial, mixed
...
gwl_combine = gwl_combine.groupby('code_bss').resample('MS').mean(numeric_only=True)
...
X_train_l_all = np.empty((0, seq_length, 7))
y_train_l_all = np.empty((0,  1))

X_valid_ls= np.empty((0, seq_length, 7))
...
# Iterate over each combination of modtype and GWL_type
for GWL_type in GWL_types:
    params_dict = {'GWL Type': GWL_type}
    for modtype in modtypes:
        # Construct the file name
        file_name = f'/media/chidesiv/DATA2/Gauged_simplified/1_layer/Multi_station/Cluster_Static_OHE/Standalone/study_{modtype}{GWL_type}.pkl'
        ...
        params_dict[modtype] = Study_DL.best_params
...
output_file = '/media/chidesiv/DATA2/Gauged_simplified/best_hyper_params/best_params_Cluster_Static_OHE_SA.csv'
...
for code in final_df['code_bss'].unique():
    ...
    scores.to_csv('scores_era5'+str(modtype)+str(GWL_type)+'.csv')
    ...
    pyplot.title((str(modtype)+' '+ "ERA5 cluster NO ohe no static"+str(code)), size=24,fontweight = 'bold')
    ...
    pyplot.savefig('./'+'Well_ID_era5_NO_ohe_no_static'+str(modtype)+str(GWL_type)+str(initm)+'{}.png'.format(code.replace('/','_')), dpi=600, bbox_inches = 'tight', pad_inches = 0.1)
