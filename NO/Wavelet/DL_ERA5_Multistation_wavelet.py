...
initm=10
test_size=0.2
GWL_type= "all seine" #annual, inertial, mixed
L=8
wavelet='la'+str(L)
import numpy as np
...
# # Assuming final_df is your DataFrame and Wavelet_transform is your wavelet function

# # Get unique codes
# unique_codes = final_df['code_bss'].unique()

# # Loop through each code
# for code in unique_codes:
#     code_data = final_df[final_df['code_bss'] == code]
    
#     # Initialize an empty DataFrame to store wavelet components
#     X_train_w = pd.DataFrame()
    
#     # Loop through each variable and perform wavelet transform
#     for variable in ['u10', 'v10', 't2m', 'e', 'msl', 'ssr', 'tp']:
#         variable_data = code_data[variable].values.reshape(-1, 1)
        
#         # Check for NaN or Infinite values
#         if np.any(np.isnan(variable_data)) or np.any(np.isinf(variable_data)):
#             print(f"Warning: NaN or Infinite values found in {variable}. Skipping.")
#             continue
        
#         try:
#             variable_w = Wavelet_transform(variable_data)
#         except Exception as e:
#             print(f"Error in wavelet transform for {variable}: {str(e)}")
#             continue
#         # print(variable_w)
#         # Add wavelet components to the DataFrame
#         for i in range(variable_w.shape[1]):
#             col_name = f'{variable}_{i+1}'
           
#             X_train_w[col_name] = variable_w[i]
    
#     # Set up subplots for each variable
#     fig, axes = plt.subplots(X_train_w.shape[1], 1, figsize=(8, 3*X_train_w.shape[1]))
    
#     # Plot wavelet components against date for each variable
#     for i, column in enumerate(X_train_w.columns):
#         axes[i].plot(code_data['date'], X_train_w[column], label=column)
#         axes[i].set_title(f'Code: {code} - {column}')
#         axes[i].set_xlabel('Date')
#         axes[i].set_ylabel('Wavelet Component')
#         axes[i].legend()
...
X_train_l_all = np.empty((0, seq_length, 7*5))
y_train_l_all = np.empty((0,  1))

X_valid_ls= np.empty((0, seq_length, 7*5))
...
import pickle
with open('/media/chidesiv/DATA2/Gauged_simplified/1_layer/Multi_station/No_OHE_NO_static/Wavelet/'+'study_'+str(modtype)+str(GWL_type)+str(wavelet)+'.pkl','rb') as f:
...
for code in final_df['code_bss'].unique():
    ...
    pyplot.title((str(modtype)+' '+str(wavelet)+ "ERA5 NO ohe no static"+str(code)), size=24,fontweight = 'bold')
    ...
    pyplot.savefig('./'+'Well_ID_era5_NO_ohe_no_static'+str(modtype)+str(wavelet)+str(GWL_type)+str(initm)+'{}.png'.format(code.replace('/','_')), dpi=600, bbox_inches = 'tight', pad_inches = 0.1)
