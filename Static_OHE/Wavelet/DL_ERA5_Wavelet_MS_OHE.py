modtype="LSTM" #GRU, LSTM,BILSTM
seq_length=48
initm=10
test_size=0.2
GWL_type= "allFRANCE" #annual, inertial, mixed
L=8
wavelet='la'+str(L)
import numpy as np
...
import pickle
with open('/media/chidesiv/DATA2/Gauged_simplified/1_layer/Multi_station/Static_OHE/Wavelet/18575/'+'study_'+str(modtype)+str(wavelet)+str(GWL_type)+'.pkl','rb') as f:
...
# with open('/media/chidesiv/DATA2/Gauged_simplified/1_layer/Multi_station/Static_OHE/Wavelet/'+'study_'+str(modtype)+str(wavelet)+str(GWL_type)+'.pkl','rb') as f:
#     Study_DL = pickle.load(f)   
# par = Study_DL.best_params
# par_names = list(par.keys())
...
for init in range(initm):
    ...
    models[init] = tf.keras.models.load_model('best_model'+str(modtype)+str(wavelet)+str(GWL_type)+str(init)+'.h5', compile=False)
    models[init].compile(optimizer='adam', loss='mse')
# Create dictionary to store extended dataframes
# extended_dfs = {}
