...
if gpus:
  ...
  try:
    tf.config.set_logical_device_configuration(
        gpus[0],
        [tf.config.LogicalDeviceConfiguration(memory_limit=1024*1.3)])
...
X_train_l_all = np.empty((0, seq_length, one_hot.shape[1]+7))
y_train_l_all = np.empty((0,  1))

X_valid_ls= np.empty((0, seq_length, one_hot.shape[1]+7))
