   ...
   with device('/gpu:0'):    
    #     tf.config.experimental.set_memory_growth('/gpu:0', True)    
        ...
        optimizer_candidates={
            ...
            # "RMSprop":RMSprop(learning_rate=trial.suggest_float('learning_rate',1e-3,1e-2,log=True))ti
        }
...
#     lev = 4  # number of levels for decomposition
#     f = ((int(pow(2, lev)) - 1) * (L - 1)) + 1  # boundary affected coefficients

#     min_train_date = max(minimum_date, X['date'].min())

#     # Add 48 months plus f to the minimum date
...
#     # # Load the best parameters for each wavelet model type
#     # best_params = {}
#     # for modtype in ['GRU', 'LSTM', 'BILSTM']:
#     #     wavelet = "la8"  # Specify the wavelet type
#     #     file_name = '/media/chidesiv/DATA2/Gauged_simplified/1_layer/Single_station/Wavelet/' + f'study_{modtype}{wavelet}{code.replace("/", "_")}.pkl'
...
# results_df.to_csv('/media/chidesiv/DATA2/Gauged_simplified/best_hyper_params/results_summary_wavelet_single_station.csv', index=False)
...
# Codes_interest: 00847X0043/S1,00332X0007/S1, 03622X0027/PZ
Codes_interest = ['00847X0043/S1', '00332X0007/S1', '03622X0027/PZ']

# for code in final_df['code_bss'].unique():
for code in Codes_interest:
    # Extract data for the current code_bss from final_df
    #Extract Class value for each code_bss from coords dataframe
    Class = coords[coords['code_bss'] == code]['Class'].values[0]
   ...
   #Removing boundary affected coefficients in the beginning
    ...
    # Study_DL= optuna.create_study(direction='minimize',sampler=TPESampler(seed=10),study_name='study_'+str(modtype)+str(wavelet)+str(code))
    # Study_DL.optimize(func_dl,n_trials=30) 
    # # file_name = './'+'study_'+str(modtype)+str(wavelet)+'{}.pkl'.format(code.replace('/','_'))
    # # with open(file_name, 'wb') as file:
    # #     pickle.dump(Study_DL, file)
    # pickle.dump(Study_DL,open('./'+'study_'+str(modtype)+str(wavelet)+'{}.pkl'.format(code.replace('/','_')), 'wb'))
    # # Study_DL=pickle.load()
    # # import pickle

    # file_name = '/home/chidesiv/Desktop/Phase2/Reconstruction_all_stations/Corrected/ERA5_1940/Wavelet/'+'study_{}{}'.format(modtype, wavelet) + code.replace('/', '_')+'.pkl'
    # with open(file_name, 'rb') as file:
    #     Study_DL = pickle.load(file)
...
# for code in final_df['code_bss'].unique():    
    ...
    # scores.to_csv('scores_era5'+str(modtype)+str(wavelet)+'.csv')
    ...
    # rc('text', usetex = True)
     ...
     # Filter the 'val_calc_ngf' time series to start from the year 1970
    start_dat = pd.to_datetime("1970-01-01")  # Define the start date
    filtered_df = df[df['date'] >= start_dat]
    
    # Plot the filtered 'val_calc_ngf' data series
    pyplot.plot(filtered_df['date'], filtered_df['val_calc_ngf'], linestyle=':', marker='*', label=code)
    ...
    pyplot.plot( Test_index,sim, 'r', label ="simulated median test", linestyle='--', linewidth = 0.9)
    ...
    y_offset = 0.6
    # pyplot.plot( Extend_index[f+seq_length-1:],sim_extend, 'g', label ="reconstructed median", linewidth = 0.9)
    bbox_props = dict(boxstyle='square', facecolor='white', alpha=0.01)
    # Corrected code for training metrics (on the left side)
    pyplot.text(Train_index.iloc[-1] - pd.DateOffset(months=120), y_train.max() * 1.0, 
                f'KGE_tr: {KGE_tr:.2f}', fontsize=18, color='black', weight='bold', bbox=bbox_props)
    pyplot.text(Train_index.iloc[-1] - pd.DateOffset(months=120), y_train.max() * 1.0 - y_offset, 
                f'MAE_tr: {MAE_tr:.2f}', fontsize=18, color='black', weight='bold', bbox=bbox_props)
    pyplot.text(Train_index.iloc[-1] - pd.DateOffset(months=120), y_train.max() * 1.0 - 2*y_offset, 
                f'NRMSE_tr: {nrmse_tr:.2f}', fontsize=18, color='black', weight='bold', bbox=bbox_props)
    
    # Corrected code for testing metrics (on the right side)
    pyplot.text(Test_index.iloc[0] + pd.DateOffset(months=12), y_train.max() * 1.0, 
                f'KGE_te: {KGE_te.values[0]:.2f}', fontsize=18, color='black', weight='bold', bbox=bbox_props)
    pyplot.text(Test_index.iloc[0] + pd.DateOffset(months=12), y_train.max() * 1.0 - y_offset, 
                f'MAE_te: {MAE_te:.2f}', fontsize=18, color='black', weight='bold', bbox=bbox_props)
    pyplot.text(Test_index.iloc[0] + pd.DateOffset(months=12), y_train.max() * 1.0 - 2*y_offset, 
                f'NRMSE_te: {nrmse_te.values[0]:.2f}', fontsize=18, color='black', weight='bold', bbox=bbox_props)
    ...
    # pyplot.text(textbox_x, textbox_y, f'KGE_te: {float(KGE_te):.2f}', fontsize=18,  color='black', weight='bold',bbox=bbox_props)
    # pyplot.text(textbox_x, textbox_y - (y_train.max() * 0.02), f'MAE_te: {MAE_te:.2f}', fontsize=18, color='black', weight='bold', bbox=bbox_props)
    # pyplot.text(textbox_x, textbox_y - (y_train.max() * 0.04), f'NRMSE_te: {float(nrmse_te):.2f}', fontsize=18, color='black', weight='bold', bbox=bbox_props)
    
    # pyplot.fill_between( Extend_index[f+seq_length-1:],lower_bound_extend,
    #                 upper_bound_extend, facecolor = (1,0.1,0,0.2),
    ...
    # pyplot.vlines(x=Train_index[f+seq_length:].iloc[0], ymin=[y_train.min()*0.999], ymax=[y_train.max()*1.001], colors='teal', ls='-', lw=2, label='Start of Training data')
    pyplot.title((str(modtype)+' '+ "Single station :"+str(wavelet)+str(code)), size=24,fontweight = 'bold')
    pyplot.xticks(fontsize=18)
    pyplot.yticks(fontsize=18)
    pyplot.ylabel('GWL [m asl]', size=24)
    pyplot.xlabel('Date',size=24)
    pyplot.legend(loc='upper left', fontsize=24, bbox_to_anchor=(1.01, 0.8), borderaxespad=0.1)
    pyplot.tight_layout()

    pyplot.savefig('./'+'Well_ID_ERA5_rev2'+str(modtype)+str(wavelet)+str(initm)+'{}.png'.format(code.replace('/','_')), dpi=600, bbox_inches = 'tight', pad_inches = 0.1)
