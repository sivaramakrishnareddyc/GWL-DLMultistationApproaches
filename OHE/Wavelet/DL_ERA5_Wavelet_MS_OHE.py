...
X_train_l_all = np.empty((0, seq_length, one_hot.shape[1]+7*5))
y_train_l_all = np.empty((0,  1))

X_valid_ls= np.empty((0, seq_length, one_hot.shape[1]+7*5))
...
for code in final_df['code_bss'].unique():
    ...
    df = final_df1[final_df1['code_bss'] == code]
...
import pickle
with open('/media/chidesiv/DATA2/Gauged_simplified/1_layer/Multi_station/OHE/Wavelet/'+'study_'+str(modtype)+str(wavelet)+str(GWL_type)+'.pkl','rb') as f:
...
# Convert Timestamp to datetime
    ...
    pyplot.savefig('./'+'Well_ID_era5_OHE'+str(modtype)+str(wavelet)+str(GWL_type)+str(initm)+'{}.png'.format(code.replace('/','_')), dpi=600, bbox_inches = 'tight', pad_inches = 0.1)
